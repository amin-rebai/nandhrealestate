...
const BlogPost: React.FC = () => {
  ...
  // Get current language for multilingual text extraction
  ...
  useEffect(() => {
    const fetchBlogPost = async () => {
      ...
      try {
        // Fetch with admin=true to get full multilingual data for language switching
        const response = await axios.get(`${API_URL}/blog/slug/${slug}?admin=true`);
  ...
  // Extract multilingual text for current language
  const postTitle = post ? getMultilingualText(post.title, language) : '';

  console.log("first", post)
  const postExcerpt = post ? getMultilingualText(post.excerpt, language) : '';
  const postContent = post ? getMultilingualText(post.content, language) : '';
  const postCategory = post ? getMultilingualText(post.category, language) : '';
  const seoTitle = post && post.seo?.metaTitle ? getMultilingualText(post.seo.metaTitle, language) : postTitle;
  const seoDescription = post && post.seo?.metaDescription ? getMultilingualText(post.seo.metaDescription, language) : postExcerpt;
  const seoKeywords = post && post.seo?.keywords ? getMultilingualText(post.seo.keywords, language) : '';
  const ogTitle = post && post.seo?.ogTitle ? getMultilingualText(post.seo.ogTitle, language) : postTitle;
  const ogDescription = post && post.seo?.ogDescription ? getMultilingualText(post.seo.ogDescription, language) : postExcerpt;
  const tiktokTitle = post && post.seo?.tiktokTitle ? getMultilingualText(post.seo.tiktokTitle, language) : postTitle;
  const tiktokDescription = post && post.seo?.tiktokDescription ? getMultilingualText(post.seo.tiktokDescription, language) : postExcerpt;
      ...
      <article className="blog-post">
        ...
        <section className="blog-post-hero visual-enhanced">
          <div className="hero-background">
            ...
            <img
              src={getUploadUrl(post.featuredImage)}
                ...
                {post.tags.length > 0 && (
                  <div className="post-tags">
                    ...
                    <div className="tags-list">
                      {post.tags.map((tag, index) => {
                        const tagText = typeof tag === 'string' ? tag : getMultilingualText(tag, language);
                        return (
              ...
              <aside className="post-sidebar">
                ...
                <div className="author-bio">
                  ...
                  <div className="author-card">
                    {post.author.avatar && (
                      <img src={getUploadUrl(post.author.avatar)} alt={post.author.name} className="author-avatar-large" />
                    )}
                    <h5>{post.author.name}</h5>
                    {post.author.bio && (
                      <p>{getMultilingualText(post.author.bio, language)}</p>
                    )}
                  </div>
                </div>
